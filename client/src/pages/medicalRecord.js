...
import { getRecordTypeTranslation, localDateTime } from "../utils/utils";
...
const MedicalRecord = () => {
    ...
    const translation = getRecordTypeTranslation(medicalRecord?.recordType, language);

    return (
        <>
            {!loading && !medicalRecord
                ?   <Card className="flex justify-content-center align-items-center card-min-height">
                        {translations?.error?.noDataAvailable}
                    </Card>
                :   <Card
                        title={translation ? translation.translatedName : medicalRecord.recordType.recordName}
                        subTitle={
                            <>
                                ...
                                {medicalRecord.doctor &&
                                    <>
                                        ...
                                        <div
                                            ...
                                            data-pr-tooltip={translations?.medicalRecord?.providerName}
                        ...
                        {medicalRecord.files && medicalRecord.files.length > 0 &&
                            <>
                                <div className="flex flex-row align-items-center gap-2 justify-content-center">
                                    ...
                                    <h3 className="flex justify-content-center">{translations?.medicalRecord?.filesGallery}</h3>
                                </div>
